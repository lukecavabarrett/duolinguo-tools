...
(function loadClusters() {
  ...
  if (!el) return;
  try {
    const clusters: Record<string, string[]> = JSON.parse(el.textContent!);
    for (const [tag, skills] of Object.entries(clusters)) {
...
function buildChoices(correct: Word, deck: Word[], direction: string): string[] {
  ...
  const rnd = new Map(others.map(c => [c, Math.random()]));
  others.sort((a, b) => {
    const ca = getCluster(a.skill) === correctCluster ? 0 : 1;
    const cb = getCluster(b.skill) === correctCluster ? 0 : 1;
    if (ca !== cb) return ca - cb;
    const da = Math.abs(DATA.skills.indexOf(a.skill) - correctSkillIdx);
    const db = Math.abs(DATA.skills.indexOf(b.skill) - correctSkillIdx);
    if (da !== db) return da - db;
    return rnd.get(a)! - rnd.get(b)!;
  });
